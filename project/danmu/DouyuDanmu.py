...
class DouYuTVDanmuThread(threading.Thread):
    ...
    def run(self):
        # self.authToDouyuServer()
        self.dynamicGet()
        self.connectToDanMuServer()
        self.danmuWhile()

    def authToDouyuServer(self):
        # 认证服务器
        auth_dst_ip = '119.90.49.105'
        auth_dst_port = 8025
        self.sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.log("认证服务器..." + auth_dst_ip + ':' + str(auth_dst_port))
        self.sock.connect((auth_dst_ip, auth_dst_port))
        self.log("连接认证成功,发送登录请求...")
        msg = "type@=qrl/rid@=" + str(self.roomId) + "/"
        self.sendMsg(msg)
        data = self.sock.recv(1024)
        self.log('Received from login\t\t' + repr(data))
        # msg = 'type@=loginreq/username@=/password@=/devid@=1C01371705D8B13361396AE2FAD50D6F/roomid@=' + str(self.roomId) + '/\x00'
        # self.sendMsg(msg)
        # data = self.sock.recv(1024)
        # self.log('Received from login\t\t' + repr(data))
    ...
    def dynamicGet(self):
        ...
        address = '119.90.49.105'
        portid = 8025
        self.sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.sock.connect((address, int(portid)))
        devid = uuid.uuid1().hex.swapcase()
        devid = "B584A9924B7311E7830F1C1B0DAE4925"
        rt = str(int(time.time()))
        hashvk = hashlib.md5()
        vk = rt + '7oE9nPEG9xXV69phU31FYCLUagKeYt22' + devid
    ...
    def connectToDanMuServer(self):
        ...
        # 抓包:msg = 'type@=loginreq/username@=qq_NEUBLK/password@=1234567890123456/roomid@=335166/'
        msg = 'type@=loginreq/username@=/password@=/devid@=1C01371705D8B13361396AE2FAD50D1F/roomid@=' + str(self.roomId) + '/\x00'
        # msg = 'type@=loginreq/username@=/password@=/roomid@=' + str(self.roomId) + '/\x00'
        self.sendMsg(msg)
        data = self.sock.recv(1024)
        self.log('Received from login\t\t' + repr(data))
